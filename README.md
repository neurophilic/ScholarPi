...
System Architecture
Intake & Identity: Authenticate securely via ORCID iD / W3C DID. Ingest files from local storage, DOIs, or OpenAlex.

Evaluation Pipeline: Parse text using chunked LLM ensembles, compute adversarial logic scores, and apply CoARA-compliant weights.

Blockchain Consensus: Mint Soulbound Tokens (piQ), record proof-of-research signatures (eval_hash), and anchor storage CIDs to Ethereum Sepolia.

Outputs & Cartography: Generate markdown integrity dossiers, AI defense rebuttal strategies, and interactive PyVis network maps of global science.
...
Bash
git clone https://github.com/your-username/ScholarPi.git
cd ScholarPi

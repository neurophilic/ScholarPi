...
def render_bubble_chart_clean(target_author):
    ...
    # Add relational edges within macro-domains to increase map complexity
    ...
    if st.session_state["is_running"]:
        ...
        if st.button("Run Assessment Pipeline", type="primary", use_container_width=True):
            if not stake_amount:
                ...
                # Write directly to memory-safe temp directory instead of keeping bytes in state
                saved_files = []
...
with top_analytics_col1:
    ...
    if len(historical_rows) < min_blocks_required:
        ...
        # Fix: Dynamic Altair Graph Configuration ensures C1-C8 micro-variance is fully scaled and noticeable.
        if len(historical_rows) > 0:
            ...
            df_melted = df_history.reset_index().melt('Block / Epoch', var_name='Criterion', value_name='Weight')
            
            base = alt.Chart(df_melted).mark_line(point=True).encode(
                x='Block / Epoch:O',
                y=alt.Y('Weight:Q', scale=alt.Scale(zero=False)),
                color='Criterion:N',
                tooltip=['Block / Epoch', 'Criterion', 'Weight']

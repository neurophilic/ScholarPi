...
def extract_with_scilem(paper_text):
    ...
    return "scilem", {
        ...
        "opinion": opinion,
        "references": [],
        "science_field": "Computer Science > Artificial Intelligence",
        "api_failed": False,
...
5. "science_field": A single string dynamically predicting the research domain formatted strictly as 'Major Domain > Specific Subfield' (e.g., 'Physics > Quantum Mechanics' or 'Life Sciences > Genomics').
...
def evaluate_pdf_text_ensemble(text, model, text_limit, file_hash="unknown"):
    ...
    best_science_field = "Engineering & Technology > Applied Technical Research"

    title_found, author_found, field_found = False, False, False
    for l_key in ["llama", "mistral", "qwen", "gemini", "scilem"]:
        ...
        if not author_found and a_val and "N/A" not in a_val:
            best_author = a_val
            author_found = True
        if not field_found and s_val and "N/A" not in s_val and ">" in s_val:
            best_science_field = s_val
            field_found = True
            
        if title_found and author_found and field_found:
            break
    ...
    return {
        ...
        "Extracted_Science_Field": best_science_field,
...
def assess_manuscript(
    ...
    try:
        ...
        extracted_field_str = raw_data.get("Extracted_Science_Field", "Computer Science > Core Research Domain")
        ...
        cursor.execute(
            ...
            (
                ...
                logic_integrity, 0.0, json.dumps([extracted_field_str]),
                json.dumps([major_field]), extracted_author, final_score,
    ...
    return (
        ...
        [major_field], [extracted_field_str], scores_dict, file_hash, piq_minted, tx_hash, zk_proof_str,

...
from brain import PiBlockchainDataset, PiBrainLSTM, generate_rebuttal_strategy, process_single_pdf
...
from integrations import (
    ...
    fetch_core_text_by_doi,
    fetch_doi_metadata,
    fetch_semantic_scholar_pdf,
    is_likely_institution,
    search_openalex_topics,
)
from ledger import generate_blockchain_pi, get_sepolia_explorer_url, restore_state_from_web3
...
function initUI() {
    ...
    // Overlay Map Modulator (⚙️) directly into the top-left corner of the map anchor
...
try:
  ...
  client_ip = (
      headers.get("X-Forwarded-For") or headers.get("X-Real-Ip") or "127.0.0.1"
  )
  if "," in client_ip:
...
if "scilm_messages" not in st.session_state:
  st.session_state.scilm_messages = [{
      "role": "assistant",
      "content": (
          "**Welcome! I am Scilem.** Click any 🤖 button next to technical"
          " app features or terms for instant explanations."
      ),
  }]
...
if "orcid_id" not in st.session_state:
  ...
  if saved_orcid and (
      re.match(r"^\d{4}-\d{4}-\d{4}-\d{3}[\dX]$", saved_orcid)
      or "did:" in saved_orcid
  ):
    st.session_state.orcid_id = saved_orcid
    st.session_state.orcid_name = (
        "Ali Vafadar Yengejeh"
        if "8050" in saved_orcid
        else "Verified Decentralized Identity"
    )
...
if not st.session_state.is_authenticated:
  ...
  st.sidebar.markdown("---")
  st.sidebar.info(
      "Notice: Please connect your ORCID iD or DID above to unlock and use your"
      " personal Assessment History features."
  )
else:
  ...
  try:
    ...
    cur_h.execute(
        "SELECT piq_minted FROM papers_assessment WHERE user_id = ? OR user_id"
        " = '0009-0009-8456-8050'",
...
SCILEM_KNOWLEDGE_BASE = {
    "authenticate": (
        ...
        " history. Pi Quotient (piQ) is a Soulbound Token assigned strictly to"
        " this identity."
    ),
    "assessment history": (
        "Displays your authenticated assessment history and earned Pi Quotient"
        " (piQ) rewards across decentralized epochs."
    ),
    "pidyne forecast": (
    ...
    "latest assessed": (
        "Displays the 5 most recently evaluated papers globally with complete"
        " assessment scores, block hashes, zk-SNARK proofs, and piQ allocations."
    ),
    "proof-of-research": (
        "Manages decentralized consensus, ledger weights, and smart contract"
        " audit proofs. It validates evaluations directly on the blockchain."
    ),
    "adversarial logic gap": (
        "Evaluates reasoning structure and penalizes claims unsupported by"
        " evidence or counterfactual stress failures."
    ),
    "c1: originality": (
        "Semantic distance from literature corpus penalized by generative AI"
        " laundering heuristics."
    ),
    "c2: methodological rigor": (
        "Deterministic adherence to MDAR reporting standards and valid RRIDs via"
        " SciScore."
    ),
    "c3: interdisciplinary synergy": (
        "Measures cross-disciplinary integration and entropy across scientific"
        " domains."
    ),
    "c4: societal impact": (
        "Evaluates broader societal and open infrastructure contributions."
    ),
    "c5: open science": (
        "Evaluates open data, open code, and containerized reproducibility."
    ),
    "c6: literature integration": (
        "Evaluates citation polarity and integration with existing foundational"
        " literature."
    ),
    "c7: empirical density": (
        "Assesses empirical sample strength and baseline variance."
    ),
    "c8: future actionability": (
        "Evaluates future research actionability and adherence to FAIR"
        " principles."
    ),
    "pi-index": (
    ...
    "global map of science": (
        "A PyVis network cartography displaying domains and subfields of assessed"
        " papers, scaled by average weights."
    ),
    "zk-snark": (
        "Zero-Knowledge Succinct Non-Interactive Argument of Knowledge. A"
        " cryptographic proof that an evaluation occurred exactly per"
        " guidelines without revealing reviewer identity."
    ),
    "sciscore mdar": (
        "SciScore evaluates adherence to the Materials Design Analysis"
        " Reporting (MDAR) framework to ensure rigor."
    ),
    "executable reproducibility score": (
        "An audit metric calculating whether code, data, and software"
        " environments (C5 & C7) can reliably execute independent results."
    ),
}
...
if "last_analyzed_tracked" not in st.session_state:
  ...
  st.session_state.scilm_messages.append({
      "role": "assistant",
      "content": (
          "**Proactive Update:** A new manuscript has been processed! Total"
          f" analyzed papers is now **{total_analyzed_count}**."
      ),
  })
...
def refine_science_field(s):
  ...
  if any(k in s_lower for k in ["blockchain", "smart contract", "crypto", "ledger"]):
    ...
    return "Computer Science > Artificial Intelligence & Machine Learning"
      ...
      k in s_lower
      for k in ["algorithm", "software", "computation", "cyber", "data", "information"]
  ):
    ...
    return "Physics > Quantum Mechanics & Optics"
      ...
      k in s_lower for k in ["energy", "mechanics", "thermodynamics", "physics"]
  ):
    return "Physics > Applied Mechanics & Energy Systems"
  elif any(
      k in s_lower
      for k in ["polymer", "catalysis", "molecule", "chemical", "chemistry"]
  ):
    ...
    return "Medical Sciences > Biomedical Research"
      ...
      k in s_lower
      for k in ["climate", "carbon", "atmosphere", "meteorology", "earth"]
  ):
    ...
    return "Social Sciences > Economics & Quantitative Finance"
      ...
      k in s_lower for k in ["sociology", "psychology", "policy", "management"]
  ):
    return "Social Sciences > Behavioral & Policy Studies"
  elif any(
      k in s_lower
      for k in ["math", "statistics", "algebra", "probability", "calculus"]
  ):
    return "Mathematics & Statistics > Applied Mathematics & Statistics"
  elif any(
      k in s_lower
      for k in ["engineering", "robotics", "materials", "civil", "electrical"]
  ):
    return (
        "Engineering & Technology > Applied Engineering & Materials Science"
    )
  else:
    return (
        "Engineering & Technology > Applied Technical Research"
        f" ({s.title()})"
    )


def render_bubble_chart_clean(
    target_author,
    repulsion=-3000,
    spring_len=180,
    size_scale=1.5,
  ...
  if not topic_aggregates:
    topic_aggregates[
        "Computer Science > Algorithms & Software Engineering"
    ] = {
        "weight_sum": 50.0,
        "frequency": 1,
    }
  ...
  for i, major in enumerate(major_keys):
    ...
    for j, topic in enumerate(subfields):
      ...
      color_map[topic] = "#%02x%02x%02x" % tuple(
  ...
  table_html += (
      "<div class='legend-container'><table"
      " class='table-big'><thead><tr><th style='width: 15%; text-align:"
      " center;'>Color</th><th>Science Field</th><th style='text-align:"
      " center;'>Freq</th><th style='text-align: center;'>Avg"
...
def evaluation_metrics_dialog():
  ...
  try:
    ...
    cur_te.execute(
        "SELECT block_height, w1, w2, w3, w4, w5, w6, w7, w8, model_used FROM"
  ...
  st.markdown(
      f"**Adversarial Logic Gap 🤖 ($\Delta_{{Logic}}$):** Evaluates reasoning"
      " structure and penalizes claims unsupported by evidence or"
      " counterfactual stress failures.",
  ...
  criteria_list = [
      (
          ...
          r"$$ C_1 = \varpi_1 \cdot \mathcal{D}_{semantic}(P_{target},"
          r" P_{corpus}) \times (1 - \lambda_{laundering}) $$",
      ),
      (
          "C2: Methodological Rigor",
          "c2: methodological rigor",
          ...
          "2",
          (
              "Deterministic adherence to MDAR reporting standards and valid"
              " RRIDs via SciScore."
          ),
          r"$$ C_2 = \varpi_2 \cdot \mathcal{I}_{blinding} + \varpi_2 \cdot"
          r" \mathcal{I}_{randomization} + \varpi_2 \cdot"
          r" \mathcal{I}_{power\_calc} + \varpi_2 \cdot"
          r" \left(\frac{N_{RRID\_valid}}{N_{RRID\_expected} + \epsilon}\right)"
          r" $$",
      ),
      (
          "C3: Interdisciplinary Synergy",
          ...
          "3",
          (
              "Measures cross-disciplinary integration and entropy across"
              " scientific domains."
          ),
          r"$$ C_3 = \varpi_3 \cdot -\sum_{i=1}^{k} p_i \ln(p_i) $$",
      ),
      (
          "C4: Societal Impact",
          "c4: societal impact",
      ...
      (
          ...
          "5",
          (
              "Evaluates open data, open code, and containerized"
              " reproducibility."
          ),
      ...
      (
          ...
          "6",
          (
              "Evaluates citation polarity and integration with existing"
  ...
  for title, q_key, weight_val, sym, desc, formula in criteria_list:
    with st.expander(
        f"{title} ( `\\varpi_{sym}` = `{weight_val:.6f}` ):",
        expanded=(title.startswith("C1")),
    ):
...
with col_t1:
  st.markdown(
      "<h1 style='margin-bottom:0;'>Pi-Index Assessment Engine 🤖</h1>",
      unsafe_allow_html=True,
  )
with col_t2:
  if st.button(
      "Evaluation Metrics, SciScore & Logic Engine", use_container_width=True
  ):
...
with st.container(border=True):
  ...
  unified_query = st.text_input(
      ...
      placeholder=(
          "Enter research topic, DOI (e.g. 10.1038/...), or search keyword..."
      ),
  ...
  if unified_query.strip():
    ...
    if (
        re.match(r"^10\.\d{4,9}/[-._;()/:A-Za-z0-9]+$", q_str)
        or q_str.startswith("10.")
        or "doi.org" in q_str
    ):
      doi_input = q_str
      include_doi = True
      ...
      if st.button(
          "Search OpenAlex Papers for this Topic",
          key=f"unified_alex_btn_{st.session_state['reset_token']}",
      ):
        ...
        with st.spinner("Querying OpenAlex..."):
          ...
          if alex_results:
            ...
            add_log(
                f"Harvested {len(alex_results)} Open Access records from"
                " OpenAlex."
            )
            st.success(
                f"Successfully harvested {len(alex_results)} papers from"
                " OpenAlex."
            )
          else:
  ...
  if (
    ...
    def toggle_all_alex():
      ...
      for i in range(st.session_state.alex_visible_count):
        st.session_state[
            f"alex_chk_{i}_{st.session_state['reset_token']}"
        ] = is_all
    ...
    try:
      if snap_alex and not st.session_state["cancel_requested"]:
        for p in snap_alex:
          ...
          add_log(
              f"Commencing open-access resolution for OpenAlex document: {fname}"
          )
          ...
          if not pdf_bytes and p_doi:
            status_text.text(
                "Direct download restricted. Querying CORE API fallback..."
            )
            core_text = fetch_core_text_by_doi(p_doi)
            if core_text:
              pdf_bytes = create_virtual_pdf_from_text(
                  core_text, title=p.get("title", "Open Access")
              )
          ...
          if pdf_bytes:
            ...
            st.session_state["evaluated_papers_buffer"] = st.session_state[
                "evaluated_papers_buffer"
            ][:50]
            add_log(
                f"Successfully processed and recorded evaluation for {fname}"
            )
          else:
            clean_doi = (
            ...
            add_log(
                "Publisher access restriction encountered for OpenAlex target."
            )
      ...
      if (
        ...
        if not pdf_bytes:
          ...
          if core_text:
            pdf_bytes = create_virtual_pdf_from_text(
                core_text, title="DOI Target Text"
            )
        ...
        if pdf_bytes:
          ...
          add_log(
              "Publisher access blocks direct binary extraction for standalone"
              " DOI."
          )
          if err_item not in st.session_state["download_errors"]:
            st.session_state["download_errors"].append(err_item)
      ...
      if snap_files and not st.session_state["cancel_requested"]:
        ...
        for i, (fname, fpath) in enumerate(snap_files):
          ...
          add_log("Stored local assessment result to cache.")
...
def more_details_dialog(item):
  ...
  with st.expander(
      f"Ledger Data & Dossier Details ({filename})", expanded=True
  ):
    ...
    st.markdown(
        f"**zk-SNARK {rbot('zk-snark')}:** `{zk_proof}`",
        unsafe_allow_html=True,
    )
    ...
    tx_disp_val = (
        tx_hash
        if tx_hash and str(tx_hash).strip() not in ["None", ""]
        else "Not Connected / No Book / Missing PK"
    )
    if tx_url:
    ...
    st.markdown(
        f"**Executable Reproducibility Score {rbot('executable reproducibility"
        f" score')}:** `{repro_score * 100:.1f}%`",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"**SciScore MDAR Adherence {rbot('sciscore mdar')}:**"
        f" `{mdar_score * 100:.1f}%` | **Valid RRIDs:** `{rrid_count}`",
        unsafe_allow_html=True,
    )
  ...
  with st.container(border=True):
    ...
    with col_actions:
      ...
      with c_det:
        if st.button(
            "More Details",
            key=f"more_det_{index}_{eval_hash}",
            use_container_width=True,
        ):
      ...
      with c_strat:
        if st.button(
            "Suggest Defense",
            key=f"gen_strat_{index}_{eval_hash}",
            use_container_width=True,
        ):
      ...
      with c_del:
        if st.button(
            "❌",
            key=f"close_eval_{index}_{eval_hash}",
            help="Close this result",
        ):
...
if (
  ...
  if st.session_state.get("download_errors"):
    ...
    for err_idx, err_data in enumerate(st.session_state["download_errors"]):
      ...
      with err_col2:
        if st.button(
            "Close", key=f"close_err_{err_idx}_{st.session_state['reset_token']}"
        ):
...
with top_analytics_col1:
  ...
  with col_fc1:
    st.markdown(
        f"### Pidyne Forecast {rbot('pidyne forecast')}", unsafe_allow_html=True
    )
  with col_fc2:
  ...
  if len(historical_rows) < min_blocks_required:
    ...
    if (
      ...
      st.session_state.predicted_next_weights = train_pibrain_cached(
          weight_data, lookback_window
      )
    ...
    if len(historical_rows) > 0:
      df_history = pd.DataFrame(
          historical_rows, columns=["C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8"]
      )
      ...
      df_melted = df_amplified.reset_index().melt(
          "Block / Epoch", var_name="Criterion", value_name="Weight (Amplified)"
      )
    ...
    st.markdown(
        f"**High-Precision Ledger Forecast (Raw Sum ="
        f" {sum(st.session_state.predicted_next_weights):.6f}/8.0):** C1:"
        f" `{st.session_state.predicted_next_weights[0]:.5f}` | C2:"
        f" `{st.session_state.predicted_next_weights[1]:.5f}` | C3:"
        f" `{st.session_state.predicted_next_weights[2]:.5f}` | C4:"
        f" `{st.session_state.predicted_next_weights[3]:.5f}` | C5:"
        f" `{st.session_state.predicted_next_weights[4]:.5f}` | C6:"
        f" `{st.session_state.predicted_next_weights[5]:.5f}` | C7:"
        f" `{st.session_state.predicted_next_weights[6]:.5f}` | C8:"
        f" `{st.session_state.predicted_next_weights[7]:.5f}`"
    )
...
with top_analytics_col2:
  ...
  with map_title_col:
    st.markdown(
        f"### Global Map of Science {rbot('global map of science')}",
        unsafe_allow_html=True,
    )
  with map_badge_col:
    st.markdown(
  ...
  with map_container:
    st.markdown(
        "<div id='map-anchor' style='position: relative; width: 100%; height:"
        " 600px;'>",
        unsafe_allow_html=True,
    )
    ...
    with st.expander("⚙️", expanded=False):
      ...
      with mod_col2:
        ...
        mod_gravity = st.slider(
            "Central Pull (Gravity)",
            min_value=0.0,
            max_value=2.0,
            value=0.15,
            step=0.01,
  ...
  with st.expander("View Map Legend, Frequency Metrics & Leaderboard"):
    ...
    if piq_dict:
      ...
      piq_df = (
          ...
          .sort_values(by="Total piQ Earned", ascending=False)
          .reset_index(drop=True)
      )
      if search_query_top:
        q_clean = search_query_top.strip().lower()
        filtered_df = piq_df[
            piq_df["Contributing Author"].str.lower().str.contains(q_clean)
            | piq_df["Unique Author Book Address"]
            .str.lower()
            .str.contains(q_clean)
        ]
...
if st.session_state.is_authenticated:
  ...
  if user_history_rows:
    for idx, uh in enumerate(user_history_rows):
      ...
      tx_disp_val = (
          u_tx
          if u_tx and str(u_tx).strip() not in ["None", ""]
          else "Not Connected / No Book / Missing PK"
      )

      with st.expander(
          f"[{idx+1}] {u_title[:65]}... — *{u_author_clean}* (Score:"
          f" **{u_score:.2f}** | piQ: `{u_piq}` | {u_time[:16]})",
          expanded=False,
      ):
        ...
        st.write(
            f"**Block Height:**"
            f" `{u_block_height if u_block_height is not None else 'Pending'}`"
        )
        st.write(
            f"**Block Hash:**"
            f" `{u_block_hash if u_block_hash is not None else 'Pending'}`"
        )
        st.write(f"**Unique Author Book Address:** `{u_book}`")
        st.write(f"**piQ Rewards Earned:** `{u_piq} piQ`")
        st.markdown(
            f"**zk-SNARK {rbot('zk-snark')} Proof:** `{u_zk}`",
            unsafe_allow_html=True,
        )
    ...
    st.info(
        "No assessment history or rewards found linked to this authenticated ID."
    )
else:
  ...
  if not recent_papers:
    ...
    for idx, rp in enumerate(recent_papers):
      ...
      tx_disp_val = (
          r_tx
          if r_tx and str(r_tx).strip() not in ["None", ""]
          else "Not Connected / No Book / Missing PK"
      )
      ...
      with st.expander(
        ...
        st.write(
            f"**Block Height:**"
            f" `{r_block_height if r_block_height is not None else 'Pending'}`"
        )
        st.write(
            f"**Block Hash:**"
            f" `{r_block_hash if r_block_hash is not None else 'Pending'}`"
        )
        st.write(f"**Unique Author Book Address:** `{r_book}`")
        st.write(f"**piQ Minted:** `{r_piq}`")
        st.markdown(
            f"**zk-SNARK {rbot('zk-snark')} Proof:** `{r_zk}`",
            unsafe_allow_html=True,
        )
...
st.markdown("---")
st.markdown(
    f"### Proof-of-Research Blockchain Explorer {rbot('proof-of-research')}",
    unsafe_allow_html=True,
)
  ...
  if epoch_data:
    ...
    with explore_col1:
      search_query = st.text_input(
          "Enter Document Evaluation Hash, Block Hash, Paper Name, or Author Name"
          " to verify ledger record...",
    ...
    if search_btn and search_query:
      try:
        ...
        if matched_records:
          st.success(
              f"Found {len(matched_records)} matching record(s) on ledger."
          )
          for m_idx, mr in enumerate(matched_records):
            ...
            m_book = (
                "0x" + hashlib.sha256(m_author_clean.encode()).hexdigest()[:40]
            )
            m_tx_url = safe_get_sepolia_url(m_tx)

            tx_disp_val = (
                m_tx
                if m_tx and str(m_tx).strip() not in ["None", ""]
                else "Not Connected / No Book / Missing PK"
            )
            ...
            with st.expander(
              ...
              st.write(
                  f"**Block Height:**"
                  f" `{m_block_height if m_block_height is not None else 'Pending'}`"
              )
              st.write(
                  f"**Block Hash:**"
                  f" `{m_block_hash if m_block_hash is not None else 'Pending'}`"
              )
              st.markdown(
                  f"**Proof-of-Research {rbot('proof-of-research')} (PoR):**"
                  f" `{m_por}`",
                  unsafe_allow_html=True,
              )
              ...
              st.markdown(
                  f"**zk-SNARK {rbot('zk-snark')} Proof:** `{m_zk}`",
                  unsafe_allow_html=True,
              )

              if m_tx_url:
                st.markdown(
                    f"**Tx Hash (Etherscan):** [`{tx_disp_val}`]({m_tx_url})"
                )
              else:
                st.write(f"**Tx Hash:** `{tx_disp_val}`")

              st.write(
                  f"**Logic Integrity:** `{m_logic:.1f}%` |"
                  f" **Reproducibility:** `{m_repro * 100:.1f}%` | **MDAR"
                  f" Adherence:** `{m_mdar * 100:.1f}%`"
              )
          ...
          st.error(
              "No records matching that evaluation hash, block hash, paper"
              " name, or author name were found on the ledger."
          )
      except Exception as e:
    ...
    reg_url = (
        f"https://sepolia.etherscan.io/address/{REGISTRY_CONTRACT_ADDRESS}"
        if REGISTRY_CONTRACT_ADDRESS
        else "#"
    )
    st.markdown(
        "**Deployed Smart Contracts on Sepolia Etherscan:** PiQ Token Contract:"
        f" [`{PIQ_CONTRACT_ADDRESS}`]({piq_url}) | Registry Contract:"
        f" [`{REGISTRY_CONTRACT_ADDRESS}`]({reg_url})"
    )
    ...
    if recent_ledger_rows:
      ...
      for rrow in recent_ledger_rows:
        ...
        tx_disp_val = (
            rtx
            if rtx and str(tx_disp_val).strip() not in ["None", ""]
            else "Missing PK"
        )
        tx_disp = (
            f"[{tx_disp_val[:10]}...]({tx_url})"
            if rtx and tx_url
            else str(tx_disp_val)
        )
        table_data.append({
...
def framework_workflow_dialog():
  st.markdown(
      "Pi-Index filters noise and yields quantitative results strictly aligned"
      " with **Responsible Research Assessment (RRA)** and **CoARA** (Coalition"
      " for Advancing Research Assessment) guidelines.\n\n"
      "### Architecture Flowchart & Whitepaper DOI\n\n"
      "Read the foundational framework whitepaper and preprints via [Ali Vafadar"
      " Yengejeh's ResearchGate"
      " Profile](https://www.researchgate.net/profile/Ali-Vafadar-Yengejeh).\n\n"
  ...
  st.markdown(
      ...
      " padding-bottom: 5px;'>Framework Author: Ali Vafadar Yengejeh |"
      " Universita degli Studi di Milano-Bicocca</div>",
      unsafe_allow_html=True,
  )
...
with scilem_container:
  st.markdown(
      "<div id='scilem-drag-handle'><span class='robot-icon'>🤖</span> Scilem"
      " Assistant</div>",
      unsafe_allow_html=True,
  )
  ...
  if submitted_floating and floating_prompt:
    st.session_state.scilm_messages.append(
        {"role": "user", "content": floating_prompt}
    )
    ...
    if direct_answer:
      st.session_state.scilm_messages.append(
          {"role": "assistant", "content": f"{direct_answer}"}
      )
      st.rerun()
    else:
      rag_context = ""
        ...
        if os.path.exists(dataset_path):
          with open(dataset_path, "r", encoding="utf-8") as f:
            ...
            relevant_lines = [
                l
                for l in lines
                if any(t in l.lower() for t in query_terms if len(t) > 3)
            ]
      ...
      try:
        ...
        cur_rag.execute(
            "SELECT title, author_name, final_score FROM papers_assessment ORDER"
            " BY final_score DESC LIMIT 1"
        )
        top_paper = cur_rag.fetchone()
        conn_rag.close()
        if top_paper:
          few_shot_examples = (
              f"Exemplar Reference Paper: '{top_paper[0]}' by {top_paper[1]}"
              f" (Score: {top_paper[2]:.2f}/100)"
          )
      except Exception:
        pass

      scilem_sys_prompt = (
          "You are Scilem, an advanced Scientific LLM aligned with CoARA"
          " guidelines and the Pi-Index Whitepaper. Explain app features clearly"
          " and concisely. You just received a user action or query.\n\n"
      ...
      messages_for_api = [{"role": "system", "content": scilem_sys_prompt}] + [
          ...
          for m in st.session_state.scilm_messages
      ]
        ...
        if groq_client:
          ...
          for attempt in range(3):
            try:
              ...
              if any(
                  k in err_str
                  for k in ["413", "rate_limit_exceeded", "tokens", "limit", "429"]
              ):
                if attempt < 2:
                ...
                try:
                  ...
                  full_response = (
                      ...
                      + "\n\n*(Payload automatically trimmed to fit TPM rate"
                      " limits).* "
                  )
                  ...
                  full_response = (
                      "Error: Token limit exceeded and fallback failed:"
                      f" {str(second_err)}"
                  )

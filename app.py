...
class AssessmentResult:
    ...
    gaming_probability: float = 0.0  # added
...
class IRTCalibrator:
    ...
    def calibrate_judge(self, judge_responses: List[Dict]) -> Dict[str, IRTCalibration]:
        ...
        for criterion in ['originality', 'rigor', 'interdisciplinary', 'impact',
            ...
            discrimination = max(0.1, corr * 2.0)
            difficulty = np.mean(responses) / 100.0
    ...
    def adjust_score(self, raw_score: float, criterion: str) -> float:
        """Apply IRT adjustment using GRM."""
...
class SciScoreClient:
    ...
    def analyze_manuscript(self, text: str, metadata: Dict = None) -> RigorMetrics:
        ...
        blinding_patterns = [r'blinded', r'double-blind', r'single-blind', r'masked', r'concealment']
        metrics.blinding_detected = any(re.search(p, text, re.IGNORECASE) for p in blinding_patterns)

        randomization_patterns = [r'randomiz', r'randomly assigned', r'random allocation']
        metrics.randomization_detected = any(re.search(p, text, re.IGNORECASE) for p in randomization_patterns)

        sample_patterns = [r'sample size.*justif', r'power analysis', r'a priori.*sample', r'effect size']
        metrics.sample_size_justified = any(re.search(p, text, re.IGNORECASE) for p in sample_patterns)

        components = [metrics.blinding_detected, metrics.randomization_detected,
                      metrics.sample_size_justified, metrics.rrid_count > 0]
        metrics.rti_score = sum(components) / len(components) * 100.0

        standards = ['MDAR', 'ARRIVE', 'CONSORT', 'NIH']
        for standard in standards:
            metrics.standards_compliance[standard] = any(re.search(standard, text, re.IGNORECASE) for _ in [1])

        metrics.sciscore_total = (metrics.rti_score * 0.6 +
...
class MultiJudgePanel:
    ...
    def add_judge(self, judge_id: str, bias_profile: Dict = None):
        ...
        self.judge_biases[judge_id] = bias_profile or {
            'position_bias': 0.0, 'verbosity_bias': 0.0, 'self_preference': 0.0
        }
    ...
    def detect_gaming(self, scores: Dict[str, float], text: str) -> float:
        ...
        if buzzword_count > len(text.split()) * 0.05:
...
class ZKProofEngine:
    """Simplified ZK proof generator (production would use zk-SNARKs)."""
    def __init__(self):
        pass
    ...
    def generate_conflict_of_interest_proof(self, reviewer_id: str, author_ids: List[str]) -> str:
        reviewer_set = set(hashlib.sha256(f"{reviewer_id}:{i}".encode()).hexdigest() for i in range(10))
        author_sets = [set(hashlib.sha256(f"{aid}:{i}".encode()).hexdigest() for i in range(10)) for aid in author_ids]
...
class PiIndexV2:
    ...
    def assess_paper(self, pdf_bytes: bytes, filename: str,
        ...
        result = AssessmentResult(
            ...
            irt_adjusted_scores={k: self.irt.adjust_score(v, k) for k, v in aggregated_scores.items()},
            ...
            gaming_probability=gaming_prob
        )
    ...
    def _get_judge_score(self, text: str, judge_id: str, scope: str) -> Dict[str, float]:
        # In production, calls the respective LLM API; here simplified
        return {
    ...
    def _store_assessment(self, result: AssessmentResult, rigor: RigorMetrics, gaming_prob: float):
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('''
...
def create_ui():
    ...
    st.markdown("**CoARA & DORA Compliant** | **IRT-Calibrated** | **ZK-Powered Privacy**")
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📄 Assessment", "📊 IRT Calibration", "🔐 ZK Privacy", "📋 Policy Compliance"])
        ...
        if st.button("Assess Paper", type="primary"):
            if uploaded_file:
                with st.spinner("Assessing paper with multi-judge panel..."):
                    result = pi_index.assess_paper(uploaded_file.read(), uploaded_file.name, scope, orcid, email)
                ...
                with col1:
                    ...
                    st.metric("Gaming Probability", f"{result.gaming_probability*100:.1f}%")
                with col2:
    ...
    with tab3:
        ...
        """)
        st.code("""zk_proof = generate_proof("review_was_double_blind", witness, public_params)""", language="python")
    ...
    with tab4:
        ...
        st.success("✅ Legacy Metrics Removed")
        st.markdown("---")
        ...
        st.markdown("""
        Bias mitigation strategies aligned with CoARA WG TIER:
    ...
    st.markdown("---")
    st.markdown("<div style='text-align: center; color: gray;'>Pi-Index v2.0 | Framework Author: Ali Vafadar Yengejeh | Università degli Studi di Milano-Bicocca</div>", unsafe_allow_html=True)
